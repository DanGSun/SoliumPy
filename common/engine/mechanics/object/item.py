from common.engine.mechanics.object import Entity
...
class Weapon(Item):
    ...
    def hit(self):
        ...
        for npc in npcs:
            if Vector2(
                    abs(npc.rect.centerx - self.owner.rect.centerx),
                    abs(npc.rect.centery - self.owner.rect.centery)
            ).as_polar()[0] < self.damage_radius:
                self.damage(npc)
